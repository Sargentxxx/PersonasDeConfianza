...
export default function ClientDashboard() {
  ...
  return (
    <div className="min-h-screen bg-[#f8fafc] dark:bg-[#020617] text-slate-900 dark:text-slate-100 flex overflow-hidden">
        ...
        <div className="flex flex-col h-full p-8">
          ...
          <div className="mt-auto space-y-4">
            <div className="p-5 rounded-2xl bg-gradient-to-tr from-slate-900 to-slate-800 text-white relative overflow-hidden group">
              ...
              <button className="w-full py-2 bg-white/10 hover:bg-white/20 rounded-xl text-xs font-bold transition-all backdrop-blur-md">
                Contactar
              </button>
            </div>
          ...
          <div className="max-w-6xl mx-auto flex items-center justify-between">
            <div className="flex items-center gap-6">
              <button
                ...
                className="lg:hidden w-10 h-10 flex items-center justify-center rounded-xl bg-white dark:bg-slate-800 shadow-sm"
              >
            ...
            <div className="flex items-center gap-4">
              <div className="hidden sm:flex items-center bg-slate-200/50 dark:bg-white/5 rounded-2xl px-4 py-2 border border-slate-200 dark:border-white/5 group focus-within:ring-2 focus-within:ring-primary/20 transition-all">
                ...
                <input
                  ...
                  className="bg-transparent border-none outline-none px-3 text-sm font-medium w-48 group-focus:w-64 transition-all"
                />
              </div>
              <NotificationBell />
        ...
        <div className="p-8 max-w-6xl mx-auto">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-6 mb-12">
            {[
              ...
              <div
                key={i}
                className="bg-white dark:bg-slate-900/50 p-6 rounded-[2rem] border border-slate-200 dark:border-white/5 shadow-sm hover:shadow-xl hover:-translate-y-1 transition-all group"
              >
                <div
                  className={`w-14 h-14 ${stat.bg} ${stat.color} rounded-2xl flex items-center justify-center mb-6 shadow-inner group-hover:scale-110 transition-transform`}
                >
          ...
          <div className="flex flex-col gap-8">
            <div className="flex items-center justify-between mb-2">
              ...
              <Link
                href="/dashboard/client/new-request"
                className="flex items-center gap-2 px-6 py-3 bg-primary text-white text-sm font-black rounded-2xl shadow-xl shadow-primary/25 hover:scale-[1.02] active:scale-[0.98] transition-all"
              >
            ...
            {loading ? (
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6 animate-pulse">
                {[1, 2].map((i) => (
                  <div
                    key={i}
                    className="h-64 bg-white dark:bg-slate-900/50 rounded-[2.5rem] border border-slate-200 dark:border-white/5"
                  ></div>
                ))}
              </div>
            ) : requests.length === 0 ? (
              <div className="flex flex-col items-center justify-center py-20 bg-white dark:bg-slate-900/50 rounded-[2.5rem] border-2 border-dashed border-slate-200 dark:border-white/5">
                ...
                <h3 className="font-display text-xl font-black mb-2 tracking-tight">
                  No tienes solicitudes todavía
                </h3>
                <p className="text-slate-500 dark:text-slate-400 mb-8 max-w-xs text-center font-medium">
                  Comienza solicitando un representante para cualquier tarea que
                  necesites.
                </p>
                <Link
                  href="/dashboard/client/new-request"
                  className="px-8 py-4 bg-primary text-white font-black rounded-2xl shadow-lg transition-all hover:scale-105"
                >
                  Empezar ahora
                </Link>
              </div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                {requests.map((req) => {
                  ...
                  return (
                    <div
                      key={req.id}
                      className="group bg-white dark:bg-slate-900/50 rounded-[2.5rem] p-8 border border-slate-200 dark:border-white/5 shadow-sm hover:shadow-2xl transition-all relative overflow-hidden flex flex-col h-full"
                    >
                      {/* Status Badge */}
                      <div className="flex items-center justify-between mb-8">
                        <div
                          className={`flex items-center gap-2 px-4 py-1.5 rounded-full text-[10px] font-black uppercase tracking-widest ${status.color}`}
                        >
                      ...
                      <div className="mb-auto">
                        <h3 className="font-display text-2xl font-black leading-tight mb-4 group-hover:text-primary transition-colors">
                          {req.title}
                        </h3>
                        ...
                        <p className="text-sm text-slate-500 dark:text-slate-400 line-clamp-2 leading-relaxed mb-8">
                          {req.description ||
                      ...
                      <div className="flex items-center justify-between pt-6 border-t border-slate-100 dark:border-white/5">
                        <div className="flex flex-col">
                          ...
                          <span className="text-xl font-black tracking-tight text-emerald-500">
                            {req.budget ? `$${req.budget}` : "A cotizar"}
                          </span>
                        </div>
                        ...
                        <div className="flex gap-2">
                          {req.status === "completed" ? (
                            <button
                              ...
                              className="px-6 py-3 bg-emerald-500 text-white font-black text-xs rounded-2xl shadow-lg shadow-emerald-500/20 transition-all hover:scale-105 active:scale-95"
                            >
                              Finalizar
                            </button>
                          ) : (
                            <>
                              {["assigned", "in_progress"].includes(
                                ...
                                <Link
                                  ...
                                  className="w-12 h-12 flex items-center justify-center bg-primary/10 text-primary rounded-2xl transition-all hover:bg-primary hover:text-white"
                                >
                              ...
                              <button
                                ...
                                className="px-6 py-3 bg-slate-900 dark:bg-white text-white dark:text-slate-900 font-black text-xs rounded-2xl transition-all hover:scale-105"
                              >
                                Detalles
                              </button>
                            </>
                      ...
                      {req.status === "assigned" &&
                        req.budget &&
                        !req.id.includes("_X_") && ( // Mock check for payment
                          <div className="mt-4">
                            <MercadoPagoButton
                              requestId={req.id}
                              title={req.title}
        ...
        >
          ...
          >
            <div className="h-40 bg-gradient-to-br from-primary via-primary-dark to-slate-900 p-12 flex items-end relative">
              <button
                onClick={() => setSelectedRequest(null)}
                className="absolute top-8 right-8 w-12 h-12 rounded-2xl bg-white/10 hover:bg-white/20 backdrop-blur-md text-white transition-all flex items-center justify-center"
              >
              ...
              <div className="relative z-10">
                <div className="flex items-center gap-3 mb-2">
                  ...
                  <span className="text-white/50 text-[10px] font-black uppercase tracking-widest">
                    Trámite #{selectedRequest.id.slice(0, 8)}
                  </span>
                </div>
                  ...
                  <div className="flex items-center gap-10">
                    <div>
                      ...
                      <p className="text-sm font-bold text-slate-700 dark:text-white uppercase tracking-tight">
                        {selectedRequest.location?.city || "Sin definir"}
                      </p>
                    </div>
                    <div>
                      <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest mb-1">
              ...
              <div className="flex gap-4">
                ...
                {["assigned", "in_progress"].includes(
                  ...
                  <Link
                    ...
                    className="flex-[2] py-4 px-6 bg-primary text-white text-center font-black rounded-2xl shadow-xl shadow-primary/25 hover:scale-[1.02] active:scale-[0.98] transition-all flex items-center justify-center gap-3"
                  >
      ...
      {ratingTask && (
        <div
          className="fixed inset-0 z-[110] flex items-center justify-center bg-slate-950/80 backdrop-blur-lg p-6"
          onClick={() => setRatingTask(null)}
        >
          <div
            className="bg-white dark:bg-slate-900 w-full max-w-md rounded-[3rem] p-12 text-center animate-scale-in"
            onClick={(e) => e.stopPropagation()}
          >
            ...
            <h2 className="font-display text-4xl font-black mb-4 tracking-tight">
              ¡Misión cumplida!
            </h2>
            <p className="text-slate-500 dark:text-slate-400 mb-10 font-medium">
              Ayúdanos a mejorar calificando tu experiencia con este
              representante.
            </p>
            ...
            <div className="flex justify-center gap-3 mb-10">
              {[1, 2, 3, 4, 5].map((star) => (
                <button
                  ...
                  className={`text-4xl transition-all hover:scale-125 ${star <= rating ? "text-yellow-400" : "text-slate-300 dark:text-slate-700"}`}
                >
                  <span className="material-symbols-outlined text-4xl leading-none fill-current">
            ...
            <div className="flex flex-col gap-4">
              <button
                ...
                disabled={submitting}
                className="w-full py-5 bg-primary text-white font-black rounded-2xl shadow-xl shadow-primary/25 hover:scale-[1.05] active:scale-[0.95] disabled:opacity-50 transition-all"
              >
                Confirmar Trámite
              </button>
              <button
                onClick={() => setRatingTask(null)}
                className="w-full py-2 text-xs font-black text-slate-400 uppercase tracking-widest hover:text-slate-600 transition-colors"
              >
                Ahora no
              </button>
            </div>
